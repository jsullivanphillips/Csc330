...
(* CHANGE: Add a case for Shift expressions *)
      | Shift(vx, vy, e1) => case e1 of
	  							NoPoints        => NoPoints
							  | Point(x,y)      => Point(x+vx, y+vy)
							  | Line(m,b)       => Line(m, (b + vy - (m * vx)))
							  | VerticalLine(x) => VerticalLine(x+vx)
							  | LineSegment(x1, y1, x2, y2) 
							                    => LineSegment(x1+vx, y1+vy, x2+vx, y2+vy)
					          | Var s => 
								(case List.find (fn (s2,v) => s=s2) env of
	     							NONE => raise BadProgram("var not found: " ^ s)
	   							  | SOME (_,v) => eval_prog(Shift(vx, vy, v), env))
							  | Let(s,e1,e2) => eval_prog (e2, ((s, eval_prog(e1,env)) :: env))
							  | Intersect(e1,e2) => intersect(eval_prog(e1,env), eval_prog(e2, env))

(* CHANGE: Add function preprocess_prog of type geom_exp -> geom_exp *)
fun preprocess_prog(geom) =
	case geom of
		LineSegment(x1, y1, x2, y2) => if (abs(x1 - x2) < 0.00001 andalso abs(y1 - y2) < 0.0001)
									   then Point(x1, y1)
									   else if (x1 > x2) then LineSegment(x2, y2, x1, y1)
									   else if (abs(x1 - x2) < 0.00001) andalso (y2 < y1) then LineSegment(x2, y2, x1, y1)
